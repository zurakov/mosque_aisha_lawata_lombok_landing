...
export function ScheduleManager({ initial }: { initial: ScheduleEntryDTO[] }) {
  const [entries, setEntries] = useState<ScheduleEntryDTO[]>(initial);
  ...
  const refresh = async () => {
    ...
    if (res.ok) {
      const json = (await res.json()) as { entries: ScheduleEntryDTO[] };
  ...
  // Reorder within the same day by swapping `order` values.
  const move = async (entry: ScheduleEntryDTO, dir: -1 | 1) => {
    ...
    try {
      await Promise.all([
        fetch('/api/admin/schedule', {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ ...entry, order: swapWith.order }),
        }),
        fetch('/api/admin/schedule', {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ ...swapWith, order: entry.order }),
        }),
      ]);
      await refresh();
    } finally {
    ...
    <div>
      <div className="mb-4 flex items-center justify-between">
        ...
        <button
          onClick={() => {
            setError('');
            setDraft({ ...emptyDraft, order: entries.length });
          }}
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-1">
                    <button
                      ...
                      disabled={busy}
                      title="Move up"
                      className="rounded p-1.5 text-muted hover:bg-primary/5 hover:text-primary disabled:opacity-40"
                    >
                    ...
                    <button
                      ...
                      title="Edit"
                      className="rounded p-1.5 text-muted hover:bg-primary/5 hover:text-primary"
                    >
          ...
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="rounded p-1 text-muted hover:bg-primary/5"
                aria-label="Close"
              >
                <X className="h-5 w-5" />
              </button>
            ...
            <div className="mt-4 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <label className="text-sm">
                  ...
                  <select
                    value={draft.day}
                    onChange={(ev) => setDraft({ ...draft, day: ev.target.value as Day })}
                ...
                <label className="text-sm">
                  ...
                  <select
                    ...
                    onChange={(ev) => setDraft({ ...draft, audience: ev.target.value as Audience })}
                    className="mt-1 w-full rounded-lg border border-primary/20 px-3 py-2 outline-none focus:border-primary"
                  >
                    {AUDIENCES.map((a) => (
              ...
              <label className="block text-sm">
                ...
                <input
                  ...
                  placeholder="e.g. Ba'da Maghrib or 19:30 WITA"
                  className="mt-1 w-full rounded-lg border border-primary/20 px-3 py-2 outline-none focus:border-primary"
                />
              </label>
              ...
              <label className="block text-sm">
                ...
                <input
                  value={draft.topic}
                  onChange={(ev) => setDraft({ ...draft, topic: ev.target.value })}
                  placeholder="e.g. Kajian Rutin — Tafsir"
                  className="mt-1 w-full rounded-lg border border-primary/20 px-3 py-2 outline-none focus:border-primary"
                />
              </label>
              ...
              <label className="block text-sm">
                ...
                <input
                  value={draft.ustadz}
                  onChange={(ev) => setDraft({ ...draft, ustadz: ev.target.value })}
            ...
            <div className="mt-5 flex justify-end gap-2">
              <button
                onClick={() => setDraft(null)}
                className="rounded-full px-4 py-2 text-sm text-muted hover:bg-primary/5"
              >
                Cancel
              </button>
              <button
